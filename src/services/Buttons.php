...
use enupal\paypal\elements\PaypalButtons as BackupElement;
use enupal\paypal\records\PaypalButton as BackupRecord;
use enupal\paypal\models\Settings;
use enupal\paypal\enums\PaypalType;
use enupal\paypal\jobs\CreateBackup;
use enupal\paypal\contracts\BackupConfig;
use enupal\paypal\contracts\DatabaseBackup;
use enupal\paypal\contracts\DirectoryBackup;
...
class Buttons extends Component
{
    ...
    /**
     * Execute Enupal PaypalButton Job from the service layer
     *
     * @return array
     */
    public function executeEnupalPaypal()
    {
        $success = false;
        $response = [
            'success' => true,
        ...
        if (!PaypalButtons::$app->settings->isWindows()) {
            // listen by console
            $shellCommand = new ShellCommand();
            $craftPath = CRAFT_BASE_PATH;
            $phpPath = PaypalButtons::$app->backups->getPhpPath();
            $command = 'cd'.
                ' '.$craftPath;
            $command .= ' && '.$phpPath.
                ' craft'.
    ...
    /**
     * Returns a PaypalButton model if one is found in the database by backupId
     *
    ...
    {
        $query = BackupElement::find();
        $query->backupStatusId = PaypalType::RUNNING;
    ...
    public function saveBackup(BackupElement $backup)
    {
        if ($backup->id) {
            ...
            if (!$backupRecord) {
                throw new Exception(PaypalButtons::t('No PaypalButton exists with the ID “{id}”', ['id' => $backup->id]));
            }
        }
    ...
    /**
     * Performs a Enupal PaypalButton operation.
     *
     * @return boolean
     * @throws Exception
    ...
    public function enupalBackup(BackupElement $backup)
    {
        ...
        $configFile = PaypalButtons::$app->backups->getConfigJson($backup);
        // update the the backup to running
        $backup->backupStatusId = PaypalType::RUNNING;
        ...
        if (!is_file($configFile)) {
            throw new Exception("Could not create the Enupal PaypalButton: the config file doesn't exist: ".$configFile);
        }
    ...
    public function initializeBackup()
    {
        ...
        $backup->backupStatusId = PaypalType::STARTED;
    ...
    public function updateBackupOnComplete(BackupElement $backup)
    {
        ...
        if ($log) {
            ...
            // PaypalButton succesfully
            $backup->backupStatusId = PaypalType::FINISHED;
            /*
    ...
    public function sendNotification(BackupElement $backup)
    {
        ...
        $backupSettings = PaypalButtons::$app->settings->getSettings();
        ...
        if ($emailSent) {
            PaypalButtons::info('Notification Email sent successfully!');
        } else {
            PaypalButtons::error('There was an error sending the Notification email');
        }
    ...
    private function getConfigJson(BackupElement $backup)
    {
        ...
        // let's create the PaypalButton Element
        ...
        if (!$this->saveBackup($backup)) {
            throw new Exception('Unable to create the element record for the PaypalButton: '.$backupId.
        ...
        if ($settings->enableLocalVolumes) {
            if (is_array($settings->volumes)) {
                ...
                $assets = PaypalButtons::$app->settings->getAllLocalVolumesObjects();
            }
        }
        // Adding the assets
        foreach ($assets as $key => $asset) {
            ...
            if (get_class($asset) == Local::class) {
                ...
                if (is_dir($asset->path)) {
                    ...
                    PaypalButtons::error('Skipped the volume: '.$asset->id.' because the path does not exists');
                }
            }
        }
    ...
    public function deleteConfigFile()
    {
        ...
        if (file_exists($file)) {
            ...
            // File not found.
            PaypalButtons::error(PaypalButtons::t('Unable to delete the config file'));
        }
    }
    ...
    public function deleteBackup(BackupElement $backup)
    {
        ...
        try {
            ...
            if (!$success) {
                $transaction->rollback();
                PaypalButtons::error("Couldn’t delete PaypalButton on deletebackup service.");
            ...
            if ($count > $settings['backupsAmount']) {
                ...
                if ($totalToDelete) {
                    ...
                    foreach ($backups as $key => $backup) {
                        ...
                        if ($response) {
                            PaypalButtons::info('EnupalPaypal has deleted the backup Id: '.$backup->backupId);
                        } else {
                            PaypalButtons::error('EnupalPaypal has failed to delete the backup Id: '.$backup->backupId);
                        }
                    }
                }
            }
    ...
    public function applyCompress()
    {
        ...
        if (!PaypalButtons::$app->settings->isWindows() || ($settings->enablePathToTar && $settings->pathToTar)) {
            return true;
        }
    ...
    {
        $colors = [
            PaypalType::STARTED => 'white',
            PaypalType::FINISHED => 'green',
            PaypalType::RUNNING => 'blue',
            PaypalType::ERROR => 'red',
        ];
